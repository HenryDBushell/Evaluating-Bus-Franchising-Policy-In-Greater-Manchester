...
PrivateCars <- PrivateCars %>%
  filter(BodyType == "Cars" & Keepership == "Private" & Fuel == "Total") %>% ##Filters for the information in the spreadsheet that we actually want
  filter(str_detect(`ONS Code`, "E06|E10|E11")) ##Filters for only the regions that we want, i.e. ones starting containing the code E06, E10 or E11
...
PrivateCars <- PrivateCars %>%
  ...
  arrange(`ONS Geography`) %>% ##Sorts the data alphabetically by name of local/combined authority
  mutate(`Private cars licensed in each region in each year` = as.numeric(`Private cars licensed in each region in each year`)) ##Transforms the car ownership data from characters into numbers
